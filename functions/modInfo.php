...
class modInfo {
    ...
    private function getModInfos(array $modTypes, string $filePath, string $fileName): array {
        ...
        */
        ...
        if ($modTypes['neoforge']===TRUE) { // almost identical to forge
            $raw = $zip->getFromName(NEOFORGE_INFO_PATH);
            if ($raw === FALSE) {
                error_log  ('{"status": "error", "message": "Could not access info file from Neoforge mod."}');
        ...
        if ($modTypes['forge']===TRUE) { // almostidentical to neoforge
    ...
    public function getModInfo(string $filePath, $fileName) {
        ...
        foreach (array_keys($mod_infos) as $k) {
            if (!empty($mod_infos[$k])) {
                return $mod_infos[$k];
            }
        }
        return [];
