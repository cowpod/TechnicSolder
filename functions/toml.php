...
final class Toml
{
    ...
    private function actual_parse($arr)
    {
        ...
        foreach ($arr as $element) {
            if (strlen($element) > 4 && $element[0] == '[' && $element[1] == '[' && $element[-1] == ']' && $element[-2] == ']') {
            ...
            } elseif (strlen($element) > 2 && $element[0] == '[' && $element[-1] == ']') {
                // [ TABLE ]
                $table_name = ltrim($element, '[[');
                $table_name = rtrim($table_name, ']]');
                $nested_table_array = explode('.', $table_name);
                ...
                if (sizeof($keyval) == 1) {
                    ...
                    elseif (strlen($value_r) >= 1 && $value_r == '[') { // EXPECT JSON
                        // error_log('start json data');
                        $json_key = $key;
                        $json_str = '[';
                    } elseif (strlen($value_r) >= 2 && $value_r[0] == '[') { // EXPECT JSON
                        // error_log('start json data');
                        $json_key = $key;
                        $json_str = $value_r;
                    } elseif (strlen($value_r) >= 1 && $value_r == ']') { // DONE WITH JSON
                        $json_str = trim($json_str, ',');
                        $json_str .= ']';
                        ...
                        $json_key = null;
                        $json_str = null;
                    } elseif (strlen($value_r) >= 2 && $value_r[strlen($value_r) - 1] == ']') { // DONE WITH JSON
                        $json_str .= $value_r;
                        // technically not json. so needs some work.
                        $parsedjson = preg_replace('/([,{])\s*([\w$]+)\s*=\s*/', '$1 "$2": ', $json_str);
                        $parsedjson = preg_replace('/\'([^\']+)\'/', '"$1"', $parsedjson);
